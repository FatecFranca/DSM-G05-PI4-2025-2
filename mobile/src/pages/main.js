...
export default function Main() {
  ...
  const registrosComTempo = registros.filter((v) => v.horaEntrada && v.horaSaida);

  // --- Gráfico de tempo médio por intervalo ---
  const intervalos = Array.from({ length: 12 }, (_, i) => i * 2); // [0,2,4,...22]
  const temposPorIntervalo = {};

  registrosComTempo.forEach((v) => {
    const date = new Date(v.horaEntrada);
    if (!isNaN(date)) {
      const hora = date.getHours(); // hora local
      const tempo = (new Date(v.horaSaida) - date) / 60000; // minutos
  ...
  const intervalLabels = intervalos.map((h) => `${h}-${h + 2}h`);
  ...
  const horasComRegistro = Object.keys(picoHoras).map(Number).sort((a, b) => a - b);
  const dataPicos = horasComRegistro.map((h) => picoHoras[h]);
  const labelsPicos = horasComRegistro.map((h) => `${h}h`);
  const maxPico = Math.max(...dataPicos);
    ...
    <Container>
      <Content>
        ...
        >
          ...
          {/* Tempo médio por intervalo */}
          {intervalData.length > 0 && (
            <ChartCard>
              <ChartHeader>
                <ChartTitle style={{ fontFamily: fontsLoaded ? "Saira_700Bold" : "System" }}>
                  Tempo Médio de Permanência por Intervalo de 2h (min)
                </ChartTitle>
              </ChartHeader>
              <View style={{ width: "100%", alignItems: "center", justifyContent: "center" }}>
                <BarChart
                  data={{ labels: intervalLabels, datasets: [{ data: intervalData }] }}
                  ...
                  chartConfig={{
                    ...
                    propsForLabels: { rotation: 45, fontSize: 12, fontWeight: "bold" },
            ...
            <ChartCard>
              ...
              <View style={{
                ...
                {dataPicos.map((value, index) => {
                  const barraWidth = (screenWidth * 0.9 * 0.7 * value) / maxPico;
                  return (
                    <View key={index} style={{ width: "90%", marginVertical: 4, flexDirection: "row", alignItems: "center" }}>
                      <Text style={{ width: 40, fontSize: 12, fontWeight: "bold", color: "#5a5a78" }}>
                        {labelsPicos[index]}
                      </Text>
                      <View style={{
                        height: 24,
                        ...
                        <Text style={{ color: "#fff", fontSize: 10, fontWeight: "bold", textAlign: "right" }}>
                          {value}
                        </Text>
                      </View>
                    </View>
